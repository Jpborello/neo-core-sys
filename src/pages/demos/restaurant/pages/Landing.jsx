...
import { UtensilsCrossed, ArrowRight, CalendarDays, Clock, Users, X, CheckCircle } from 'lucide-react';
...
const Landing = () => {
    ...
    const [reservationId, setReservationId] = useState(null); // Track my res
    const [resName, setResName] = useState('');
    const [resPax, setResPax] = useState(2);
    const [resDate, setResDate] = useState(''); // Empty = Now
    const [resPhone, setResPhone] = useState('');
    const [resStatus, setResStatus] = useState(null); // 'success', 'waitlist', 'error'
    ...
    const handleReservation = async (e) => {
        ...
        // 1. Check Availability (Simple Logic: Count occupied tables)
        ...
        const isFull = (count || 0) >= TOTAL_TABLES;
        const isNow = !resDate;
        ...
        if (isNow && isFull) {
            status = 'waiting';
            note = 'Lista de Espera - Restaurante Lleno';
        }
        ...
        const { data, error } = await supabase.from('reservations').insert([{
            ...
            party_size: resPax,
            reservation_time: resDate ? new Date(resDate).toISOString() : null,
        ...
        if (error) {
            ...
            setResStatus(status === 'waiting' ? 'waitlist' : 'success');
        ...
        <div className="min-h-screen bg-neutral-900 text-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
            ...
            <div className="relative z-10 w-full max-w-md space-y-8 animate-fade-in-up">
                ...
                {!showReservation ? (
                    ...
                    <div className="bg-neutral-800/50 backdrop-blur-sm p-8 rounded-2xl border border-neutral-700 shadow-xl relative animate-in fade-in zoom-in-95">
                        ...
                        {!resStatus ? (
                            <form onSubmit={handleReservation} className="space-y-4">
                                <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><CalendarDays className="text-amber-500" /> Reservar Mesa</h2>

                                <div>
                                    <label className="block text-xs font-bold text-neutral-500 uppercase mb-2">Nombre</label>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-neutral-500 uppercase mb-2">Horario</label>
                                        <input type="datetime-local" value={resDate} onChange={e => setResDate(e.target.value)} className="w-full bg-neutral-900 border border-neutral-700 rounded-xl p-3 text-xs text-white focus:border-amber-500 outline-none" />
                                        <span className="text-[10px] text-neutral-500 block mt-1">Dejar vacío para "Ahora"</span>
                                    </div>
                                </div>

                                <div>
                                    <label className="block text-xs font-bold text-neutral-500 uppercase mb-2">Teléfono</label>
                                    <input type="tel" required value={resPhone} onChange={e => setResPhone(e.target.value)} className="w-full bg-neutral-900 border border-neutral-700 rounded-xl p-3 text-white focus:border-amber-500 outline-none" placeholder="Para confirmar reserva" />
                                </div>
                            ...
                            <div className="text-center py-6">
                                <div className="w-20 h-20 bg-emerald-500/20 rounded-full flex items-center justify-center mx-auto mb-4 text-emerald-500">
                                    <CheckCircle size={40} />
                                </div>
                                <h3 className="text-xl font-bold text-white mb-2">¡Reserva Confirmada!</h3>
                                <p className="text-neutral-400 text-sm mb-6">Te hemos enviado los detalles por WhatsApp. ¡Te esperamos!</p>
                                <button onClick={() => setShowReservation(false)} className="bg-neutral-700 hover:bg-neutral-600 px-6 py-2 rounded-lg text-sm font-bold">Volver</button>
                            </div>
                        ) : resStatus === 'waitlist' ? (
                            <div className="text-center py-6">
                                ...
                                <h3 className="text-xl font-bold text-white mb-2">No hay mesas disponibles</h3>
                                <p className="text-neutral-400 text-sm mb-6">Te hemos sumado a la <strong>Lista de Espera</strong>. Te contactaremos a tu WhatsApp apenas se libere una mesa.</p>
                                <button onClick={() => setShowReservation(false)} className="bg-neutral-700 hover:bg-neutral-600 px-6 py-2 rounded-lg text-sm font-bold">Entendido</button>
                            </div>
                        ) : (
                            <div className="text-center py-6">
