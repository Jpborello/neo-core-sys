...
const Menu = () => {
    ...
    const { addToCart, cart, products, loadingProducts, requestAssistance, requestBill, tableState, requestPayCash } = useRestaurant();
    const [activeCategory, setActiveCategory] = useState('all');
    const [search, setSearch] = useState('');
    const [assistanceRequested, setAssistanceRequested] = useState(false);
    const [billRequested, setBillRequested] = useState(false);

    // Sync local state with remote state for button feedback
    React.useEffect(() => {
        if (tableState) {
    ...
    // Payment Modal (Triggered by Waiter closing table)
    // Payment Modal (Triggered by Waiter closing table)
    const [orderHistory, setOrderHistory] = useState([]);
    const [tipAmount, setTipAmount] = useState(0);
    const [showTipInput, setShowTipInput] = useState(false);
    const { fetchTableOrders } = useRestaurant();
                ...
                {filteredProducts.map(product => (
                    ...
                    >
                        <div className="w-1/3 h-full relative">
                            <img
                                src={product.image || "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?auto=format&fit=crop&w=500&q=60"}
                        ...
                        <div className="w-2/3 p-4 flex flex-col justify-between">
                            <div>
                                ...
                                <p className="text-xs text-neutral-400 line-clamp-2 mb-2">{product.desc || ''}</p>
                            </div>
                            <div className="flex justify-between items-center mt-2">
                                <span className="font-bold text-amber-500">${product.price.toLocaleString()}</span>
                                <button
